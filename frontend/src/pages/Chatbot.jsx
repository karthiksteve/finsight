    ...
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(fileNames) && fileNames.length > 0) {
          setUploadedDocs(fileNames.map(name => ({ name, size: 'Indexed', url: '#' })));
          setMessages([
            {
              id: 1,
              text: `Hello! I'm CAG (Contextual AI Guide). I have "${fileNames[0]}" indexed and ready. Ask me any financial or legal questions about it!`,
              sender: 'bot',
              timestamp: new Date()
            }
          ]);
